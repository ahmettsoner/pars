resource "libvirt_volume" "loadbalancer_volume" {
  count          = var.vm_count
...
resource "libvirt_cloudinit_disk" "loadbalancer_commoninit" {
  count     = var.vm_count
  name      = "k8s_commoninit_loadbalancer-${count.index}.iso"
  pool      = var.vm_pool
...
resource "libvirt_domain" "loadbalancer_vms" {
  ...
  disk {
    volume_id = libvirt_volume.loadbalancer_volume[count.index].id
  }

  cloudinit = libvirt_cloudinit_disk.loadbalancer_commoninit[count.index].id
...
resource "local_file" "ansible_inventory" {
  depends_on = [libvirt_domain.loadbalancer_vms]
  filename = "${path.root}/files/${var.module_name}/ansible/inventory.ini"

  content = join("\n", ["[loadbalancer_servers]"], flatten([
    for vm in libvirt_domain.loadbalancer_vms : (
...
resource "null_resource" "ansible_playbook" {
  ...
  provisioner "local-exec" {
    ...
    ansible-playbook -i ${local_file.ansible_inventory.filename} ../ansible/k8s.yml --tags 'loadbalancer'
EOT
  }
