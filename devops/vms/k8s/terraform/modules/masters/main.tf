...
locals {
  ip_addresses = [
    for i in range(var.vm_count) : replace(var.vm_ip_suffix, "xxx", tostring(i + 201))
  ]
}
...
resource "libvirt_cloudinit_disk" "master_commoninit" {
  count     = var.vm_count
  name      = "k8s_commoninit_master-${count.index}.iso"
  pool      = var.vm_pool
...
resource "libvirt_domain" "master_vms" {
  count  = var.vm_count
...
resource "null_resource" "ansible_playbook" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -i ${local_file.ansible_inventory.filename} ../ansible/k8s.yml --tags 'master'
    EOT
  }
}
