...
func (s ApplicationProjectEngine) Validate(data []structs.Schema) bool {
...
func (s ApplicationProjectEngine) Process(data []structs.Schema) error {
	...
	for _, item := range data {
		...
		if err := s.completeProjectInformation("", applicationproject); err != nil {
			return err
		}
...
func (s ApplicationProjectEngine) Destroy(data []structs.Schema) error {
	applicationprojects := make([]applicationprojectStruct.ProjectBaseStruct, 0, len(data))
...
func (s ApplicationProjectEngine) completeProjectInformation(workspaceName string, project *applicationprojectStruct.ProjectBaseStruct) error {
...
func (s ApplicationProjectEngine) getWorkspace(defaultWorkspaceName string, project applicationprojectStruct.ProjectBaseStruct) (*workspaceStruct.WorkspaceBaseStruct, error) {
